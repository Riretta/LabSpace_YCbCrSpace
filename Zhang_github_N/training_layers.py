...
import torch
import faiss
quantization_file = 'pts_in_hull_ycbcr_500.npy'
...
class NNEncLayer(object):
    ...
    def __init__(self):
        ...
        self.nnenc = NNEncode(self.NN, self.sigma, km_filepath=os.path.join(self.ENC_DIR, quantization_file)) #'pts_in_hull.npy'))
        print(self.nnenc.nbrs2)
        self.X = 224
        self.Y = 224
        self.Q = self.nnenc.K
...
# NNEncode(32, 0.5, km_filepath=os.path.join(self.ENC_DIR, 'pts_in_hull.npy'))
class NNEncode():
...
def decode(data_l, conv8_313, folder,batch,epoch,rebalance=1):#2.606
    ...
    for i in range(len(data_l)):
        ...
        cc = np.load(os.path.join(enc_dir, quantization_file)) #'pts_in_hull.npy'))
        data_ab = cc[class8[:][:]]
